...
ctrl = D_controller()
# F = LSF()
reference = signalGenerator(amplitude=.25, frequency=0.05, y_offset=0.25)
...
while t < P.t_end:  # main simulation loop
    ...
    while t < t_next_plot: # updates control and dynamics at faster simulation rate
        u,est_error = ctrl.u(ref_input, msd.outputs())  # Calculate the control value
        msd.propagateDynamics(u)  # Propagate the dynamics
        t = t + P.Ts  # advance time by Ts
    # update animation and data plots
    animation.drawAll(msd.states())
    dataPlot.updatePlots(t, ref_input, msd.states(), u, est_error)
